...
> **DEBUG COPY** - Test fixes here before applying to main codebase
> 
> **For Fix Workflow, see:** `DEBUG/fix/debugdoc.md`
...
3. [Pending Fixes](#3-pending-fixes)
...
# Test resize in debug build (should work)
./debug.sh run
# Try resizing - should be fine

# Test resize in release build
./debug.sh release
# Try resizing - likely crashes

# Capture crash output
./target/release/ascii-cam-debug 2>&1 | tee resize_crash.log
# Resize multiple times, Ctrl+C
# Check: cat resize_crash.log

# Test with opt-level 2 (less aggressive)
# Edit Cargo.toml: opt-level = 2
./debug.sh release
# Try resizing
```

**Files to Modify for Testing:**
- `src/main.rs` - Add guards, debug output
- `Cargo.toml` - Change opt-level for testing
...
1. Create directory: `DEBUG/fix/<fix-name>/`
2. Add `fix.md` with documentation
3. Add modified source files in `src/`
4. Test thoroughly
5. When ready, merge to main `src/`

See `DEBUG/fix/README.md` for detailed workflow.
...
**All fixes must:**
1. Be tested in `DEBUG/` environment
2. Pass both debug AND release builds
3. Be documented in `DEBUG/fix/<name>/`
4. Update this TROUBLESHOOTING.md if new issue

**Quick merge checklist:**
- [ ] Debug build: `cargo build`
- [ ] Release build: `cargo build --release`
- [ ] Fix tested and working
- [ ] `DEBUG/fix/<name>/` created
