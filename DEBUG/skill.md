...
> **NOTE:** This is a DEBUG COPY of the main ascii-cam project.
> Make changes here to test fixes before applying to main codebase.
> Use `debug.sh` to build and test.
...
### When Investigating a New Issue

1. **Check this directory**: `ls DEBUG/` - all debugging happens here
2. **Reproduce the issue**: Run the binary, note exact error
3. **Isolate**: Disable features, test minimal case
4. **Research**: Check TROUBLESHOOTING.md, skill.md, dependency docs
5. **Fix**: Make minimal change in DEBUG/src/main.rs
6. **Test**: `./debug.sh build && ./debug.sh run`
7. **Verify**: Test both debug and release builds
8. **Document**: Create fix in `DEBUG/fix/<fix-name>/`
9. **Apply**: Copy fixed files to parent directory if successful

### Fix Workflow (NEW)

All fixes MUST go through `DEBUG/fix/` before merging:

```
┌────────────────────────────────────────┐
│ 1. Identify issue in main codebase     │
└─────────────────┬──────────────────────┘
                  ▼
┌────────────────────────────────────────┐
...
└─────────────────┬──────────────────────┘
                  ▼
┌────────────────────────────────────────┐
│ 3. Document: fix.md + src/ files     │
└─────────────────┬──────────────────────┘
                  ▼
┌────────────────────────────────────────┐
│ 4. Test: debug.sh build && run       │
└─────────────────┬──────────────────────┘
                  ▼
┌────────────────────────────────────────┐
│ 5. Verify: debug + release builds     │
└─────────────────┬──────────────────────┘
                  ▼
┌────────────────────────────────────────┐
│ 6. Merge: Copy to src/ if success   │
└────────────────────────────────────────┘
```
...
Before marking issue as resolved:
- [ ] Debug build works
- [ ] Release build works (try `./debug.sh release`)
- [ ] No regression in other features
- [ ] Documentation updated
- [ ] Fix documented in `DEBUG/fix/<name>/`
- [ ] TROUBLESHOOTING.md updated if new issue
...
./target/debug/ascii-cam-debug 2>&1 | tee crash.log

# Test with different opt-level
# Edit Cargo.toml [profile.release] opt-level = 2
./debug.sh release
```
...
| Issue | Try This |
|-------|---------|
| Release crash | Set `opt-level = 2` in Cargo.toml |
| Camera error | Check v4l2-ctl, permissions |
| Terminal glitch | Try alacritty or kitty |
| Audio no work | Check PulseAudio |
| Crash on resize | Add size guards in render loop |
